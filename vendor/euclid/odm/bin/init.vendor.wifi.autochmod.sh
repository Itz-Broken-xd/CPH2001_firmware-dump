...
function parseSupportrusEntityConfigXml() {
    ...
    local chipid=`getprop vendor.connsys.adie.chipid`
    if [ "$isConfigXmlParseDone" = "false" ]; then
        ...
        do
            ...
            if [[ $board == *"mt"* ]] || [[ $board == *"Mt"*  ]] || [[ $board == *"MT"*  ]];then
                ...
                typeIdx=$?
                if [[ "$platform" = "$board" ]] && ([[ $platform != "mt6779" ]] || [[ $platform = "mt6779"  &&  $chipid = "0x6635"  &&  $versionFileName = "WIFI_RAM_CODE_soc2_0_3_1.bin" ]] || [[ $platform = "mt6779"  &&  $chipid = "0x6631"  && $versionFileName = "WIFI_RAM_CODE_soc2_0_3a_1.bin" ]] || [[ $platform = "mt6779" && $type = "wifi.cfg" ]]) ;then
...
function excuteSarCommand() {
    ...
    local wlangen=`getprop ro.vendor.wlan.gen`
    if [ $wlangen = "gen3"  ];then
        /odm/bin/iwpriv_vendor wlan0 driver O-SAR-ENABLE\ $sarindex
    else
